...
tidyverse (Collection of packages): https://www.tidyverse.org/packages/
...
```{r}
sqldf("SELECT * FROM amazon_df")
```
In this simple query, we simply SELECTed all attributes FROM our dataset amazon_df. The star (*) means "all" in SQL. This did not really do much, since all it did was give us back amazon_df. However, we can make it do more interesting things. Let's try something more interesting. 
...
While we did a lot of data cleaning and data analysis, there is still much more we could do. For example, we could scrape spectrum and compare their top languages to those used by Amazon. https://spectrum.ieee.org/ns/IEEE_TPL_2017/index/2017/1/1/1/1/1/50/1/50/1/50/1/30/1/30/1/30/1/20/1/20/1/5/1/5/1/20/1/100/
